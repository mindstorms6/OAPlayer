		...
		public void run() {
			try {
				...
				for (int i = 0; i < NTP_ROUNDS; i++) {
					boolean didUpdate = ntpClient.requestTime(
							"tick.mtnlion.com", 30000);
					if (didUpdate) {
						long nowMillis = ntpClient.getNtpTime() + System.nanoTime()
								- ntpClient.getNtpTimeReference();
						logger.debug("Now is " + nowMillis);
						long deltaMillis = (long) (System.currentTimeMillis() - nowMillis);
						float tempNR = (float) ((float) (thisRunDeltaAvg * thisRunSuccesses) + deltaMillis)
				...
				if (thisRunSuccesses > 0) {
					...
					logger.debug(String.format(
							"New avg off delta is %d nanos.", runningDelta));
					logger.debug(String
