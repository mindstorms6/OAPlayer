...
public class MQTTManager implements MqttCallback
{
   ...
   private synchronized boolean setupClient()
   {
      ...
      else
      {
         ...
         try
         {
            ...
            MqttDefaultFilePersistence perst = new MqttDefaultFilePersistence("/tmp");
            this.client = new MqttClient(makeURI(), MQ_ID, perst);
         ...
         public void run()
         {
            while (true)
            {
               ...
               try
               {
                  ...
                  MQTTManager.this.client.publish(toSend.topic, toSend.message);
   ...
   {
      String toReturn =
         String.format("%s://%s:%d", PropertyManager.getStringForKey(PropertyKey.MQTT_SCHEME, "tcp"),
            PropertyManager.getStringForKey(PropertyKey.MQTT_HOST, "oa.bdawg.org"),
            PropertyManager.getIntForKey(PropertyKey.MQTT_PORT, 1883));
